...
public class RedisSocketFactory extends SocketFactory {
	...
	public Socket createSocket(String host, int port) throws IOException {
		if (configuration.isSsl()) {
			return buildSsl(build(configuration.getSslSocketFactory()
				.createSocket(host, port)), host);
		} else {
	...
	public Socket createSocket(String host, int port, InetAddress localAddr, int localPort) throws IOException {
		if (configuration.isSsl()) {
			return buildSsl(build(configuration.getSslSocketFactory()
				.createSocket(host, port, localAddr, localPort)), host);
		} else {
	...
	public Socket createSocket(InetAddress address, int port) throws IOException {
		if (configuration.isSsl()) {
			return buildSsl(build(configuration.getSslSocketFactory()
				.createSocket(address, port)), address.getHostAddress());
		} else {
	...
	public Socket createSocket(InetAddress address, int port, InetAddress localAddr, int localPort) throws IOException {
		if (configuration.isSsl()) {
			return buildSsl(build(configuration.getSslSocketFactory()
				.createSocket(address, port, localAddr, localPort)), address.getHostAddress());
		} else {
	...
	public Socket createSocket(String host, int port, int timeout) throws IOException {
		...
		if (configuration.isSsl()) {
			socket = configuration.getSslSocketFactory()
	...
	private Socket buildSsl(Socket socket, String host) throws SocketException {
		...
		if (configuration.getHostnameVerifier() != null && !configuration.getHostnameVerifier()
			.verify(host, ((SSLSocket) socket).getSession())) {
